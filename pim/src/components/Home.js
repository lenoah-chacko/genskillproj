...
export default function Home({searchfunc,Notes, setKeyWord,empty}) {
        ...
        <div className="homebg page">
            <div className="container pt-4 pb-5">
                    <div className="container-fluid">
                        ...
                        {Notes.length>0?
							Notes.map(note=>(
							<Link to={'/note/'+note.notesid} key={note.notesid} style={{ textDecoration: 'none' }}>
								<NoteView key={note.notesid} Title={note.title} DateCreated={note.date}>
								</NoteView>
							</Link>
						))
						:<h4 className="mt-5 text-white text-center">{empty}</h4>}
                    </div>
            </div>
        </div>
    )
